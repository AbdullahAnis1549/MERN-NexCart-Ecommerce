...
Both the **frontend** and **backend** are deployed on [Vercel](https://vercel.com) as two separate projects from the same repository.
 
### Backend (Vercel)
1. Create a new Vercel project, import this repo, set **Root Directory** to `backend`
2. Framework Preset: **Other**
3. Vercel will use the included `vercel.json` + `api/index.js` to run the Express app as a serverless function
4. Add all backend environment variables (see [Backend setup](#2-backend-setup)) in the Vercel project settings
5. Deploy — note the resulting URL (e.g. `https://your-backend.vercel.app`)
### Frontend (Vercel)
1. Create a second Vercel project, import the same repo, set **Root Directory** to `frontend`
2. Add environment variable `VITE_API_BASE_URL` = the backend URL from above
3. Deploy — note the resulting URL (e.g. `https://your-frontend.vercel.app`)
### Final step
Go back to the **backend** Vercel project → Environment Variables → set `FRONTEND_URL` to the frontend's URL, then redeploy (required for CORS to allow requests from the live frontend).
